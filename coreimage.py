...
class CImage(object):
    def __init__(self, img_or_path=None):
        ...
        if isinstance(img_or_path, ui.Image):
            self.ci_img = CIImage.imageWithCGImage_(
                ObjCInstance(img_or_path).CGImage())
    ...
    def filter(self, name, **params):
        ...
        for key, value in params.items():
            ...
            if key not in input_keys:
                raise ValueError(
                    '"%s" is not a valid parameter for %s' % (key, name))
            param_type = input_key_types[key]
            if param_type not in supported_types:
                raise TypeError(
                    'Parameters of type %s are not supported' % param_type)
    ...
    def _uiimage_rep(self, rotate=0):
    ...
    def get_PIL(self):
        temp_path = os.path.join(tempfile.gettempdir(), 'cimage_pil.png')
        saved = self.save_png(temp_path)
        if saved:
