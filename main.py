...
class muon():
	def __init__(self):
		...
		methods=[self.captureOutput_didOutputSampleBuffer_fromConnection_],
		...
		self.latestPhotoView.center = (self.mainView.width*0.12,self.mainView.height*0.85)
		...
		self.savingPhotoView.center = (self.mainView.width*0.12,self.mainView.height*0.85)
		...
		openPhotoapp.center = (self.mainView.width*0.12,self.mainView.height*0.85)
	...
	def pilSaveAlbulm(self,pilImg):
		#temp_path = os.path.join(tempfile.gettempdir(), 'piltemp.png')
	...
	def openPIL(self,fileName='output.jpg'):
		print('[LOG]Opening...')
		pilImg = Image.open('output.jpg')
		print('[LOG]Opened')
		print('[LOG]Showing...')
		pilImg.show()
		print('[LOG]Showed')
	...
	def captureOutput_didOutputSampleBuffer_fromConnection_(self, _self, _cmd, _output, _sample_buffer, *args):
		...
		#self.fps_counter += 1
		#now = time.time()
		#self.buf = _sample_buffer
		#if int(now) > int(self.last_fps_time):
			#self.label_fps.text = '{:5.2f} fps'.format((self.fps_counter) / (now - self.last_fps_time))
		...
		if self.take_photo_flag == True:
			...
			self.ciimage = CIImage.imageWithCVPixelBuffer_(ObjCInstance(imagebuffer))
